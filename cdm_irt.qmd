...
# ---- Build simulation parameters for a given K and items-per-skill ------
make_params <- function(K, ips) {
  J    <- K * ips
  slip <- rep(0.15, J)
  guess <- rep(0.15, J)

  # Q-matrix: ips items per skill, each requiring exactly one skill
  Q <- do.call(rbind, lapply(seq_len(K), function(k) {
    r <- rep(0L, K); r[k] <- 1L
    matrix(rep(r, ips), nrow = ips, ncol = K, byrow = TRUE)
  }))
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("K", "Number of skills (K):",
                  ...
                  selected = 3),
      selectInput("ips", "Items per skill:",
                  choices = c("2" = 2, "3" = 3, "4" = 4, "5" = 5),
                  selected = 3),
      helpText("Total items J = K × items per skill."),
...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    pr       <- make_params(as.integer(input$K), as.integer(input$ips))
