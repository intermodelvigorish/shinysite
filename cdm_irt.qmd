...
**Simulation design.** Data are generated from a **DINA model** (Deterministic Input, Noisy And-gate) with $K$ skills and $J = 3K$ items (three items per skill, each requiring exactly one skill). Slip and guess parameters are fixed at 0.15. Skills are drawn from a multivariate normal and thresholded to binary, with inter-skill correlation $\rho$ varied across 10 evenly-spaced values from 0 to 0.7.
...
# ---- Build simulation parameters for a given K --------------------------
make_params <- function(K) {
  J    <- K * 3L
  slip <- rep(0.15, J)
  guess <- rep(0.15, J)

  # Q-matrix: 3 items per skill, each requiring exactly one skill
  Q <- do.call(rbind, lapply(seq_len(K), function(k) {
    r <- rep(0L, K); r[k] <- 1L
    matrix(rep(r, 3), nrow = 3, ncol = K, byrow = TRUE)
  }))
...
# ---- Helper functions (all take a params list) --------------------------
gen_attrs <- function(N, rho, pr) {
  Sigma <- matrix(rho, pr$K, pr$K); diag(Sigma) <- 1
...
sim_one <- function(N_train, rho, pr) {
  ...
  # Step 1: training data — fit IRT item parameters
  a_tr <- gen_attrs(N_train, rho, pr)
  ...
  # Step 2: test data — estimate person-level parameters
  a_te   <- gen_attrs(N_test, rho, pr)
  p_true <- dina_prob(a_te, pr)
  y_te   <- matrix(rbinom(N_test * pr$J, 1L, p_true), N_test, pr$J)

  # IRT: MLE theta from y_te, then compute predicted probabilities
  ...
  # CDM: MAP attribute profiles from y_te
  p_cdm <- dina_prob(map_dina(y_te, pr), pr)

  # Step 3: evaluation data — independent draw from same true probabilities
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("K", "Number of skills (K):",
                  choices = c("2" = 2, "3" = 3, "4" = 4),
                  selected = 3),
      selectInput("N_train", "Training sample size (N):",
                  choices = c("250" = 250, "500" = 500, "1000" = 1000),
                  selected = 500),
      helpText("J = 3K items total (3 items per skill)."),
      actionButton("run", "Run Simulation", class = "btn-primary"),
      hr(),
      helpText(
        "First run installs ltm (~30s). Simulation sweeps 10 values of ρ ",
        "and fits a 2PL IRT model at each; expect 1–3 minutes."
      )
    ),
    mainPanel(
...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    pr       <- make_params(as.integer(input$K))
    N_train  <- as.integer(input$N_train)
    rho_vals <- seq(0, 0.7, length.out = 10)
    ...
    list(rho = rho_vals, imv = out, K = pr$K, J = pr$J)
  })
  ...
  output$imvPlot <- renderPlot({
    ...
    ylim <- range(c(imv, 0), na.rm = TRUE)
    ylim <- ylim + diff(ylim) * c(-0.08, 0.08)

    plot(rho, imv[, "irt_cdm"],
         type = "n",
         xlab = expression(paste("Skill inter-correlation (", rho, ")")),
         ylab = "IMV",
         main = sprintf("IMV vs Skill Correlation  (N = %s, K = %d, J = %d)",
                        input$N_train, r$K, r$J),
         ylim = ylim, las = 1)
