...
In this example, we consider the logistic regression model $$\Pr(y=1)=\frac{1}{1+\text{exp}(-(b_0+b_1 x))}$$. We generate $y$ based on a sample of 5000 $x$ values and consider the IMV associated with predicting new $y$ outcomes (based on new $x$ observations for different values of $b_0$ and $b_1$. As we might expect the IMV is always near zero when $b_1$ is near zero. However,  as $b_0$ gets large, increases in $b_1$ lead to less significant predictive gains given that there is substantially less uncertainty in the outcomes. 
...
ui <- fluidPage(
  titlePanel("Logistic Regression: om vs b1"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("Adjust b0 and click 'Run Simulation' to see how om varies with b1.")
    ),
...
server <- function(input, output, session) {
  ...
  output$omPlot <- renderPlot({
    ...
    plot(results$b1vals, results$om,
         ...
         ylab = "om",
         ylim=c(0,.8),
         main = paste0("om vs b1 (b0 = ", input$b0, ")"),
         pch = 19,
         col = "steelblue")
