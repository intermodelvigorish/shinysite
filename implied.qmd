...
We illustrate three things here. In Tab 1, we show the **category response functions** for all three models (GRM, PCM, SRM) using the item parameters implied by the DGM choice. In Tab 2, we show the **implied probability curves** for every possible dichotomization, comparing how the three models differ. In Tab 3, we compute **IMV(DGM, DAM)** — how much better the DGM's predictions are relative to the DAM's, across all dichotomizations — using a fresh set of out-of-sample responses.
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("spread", "Threshold spread:",   min = 0.3, max = 2.5, value = 1.0, step = 0.1),
      ...
      p("N = 2000 persons, \u03b8 ~ N(0,1). Both DGM and DAM parameters estimated from first set of responses; IMV computed on a fresh set of responses from the estimated DGM.",
      ...
      tabsetPanel(
        ...
        tabPanel("Category response functions",
          ...
          p("The plots below show the analytical category response functions (CRFs) —
            ...
            using a single set of item parameters: the discrimination a and thresholds b
            (evenly spaced and scaled by the spread slider) as specified above.
            These are not estimated from data; they describe what each model predicts
            for a single item with known parameters. Despite differing in construction,
            the three models produce strikingly similar CRFs for typical parameter values,
            motivating examination of the full set of implied probabilities (Tab 2)
            as a more sensitive discriminator."),
        ...
        tabPanel("IMV comparison",
          br(),
          p("Click 'Run simulation' to: (1) draw \u03b8 ~ N(0,1) for N = 2000 persons and simulate
            item responses from the DGM; (2) estimate item parameters for both the DGM and DAM
            from those responses using the known \u03b8 values; (3) generate a fresh set of N = 2000
            responses from the DGM using its estimated parameters; (4) compute IMV(DGM, DAM)
            for every dichotomization by comparing DGM predictions to DAM predictions on the
            new responses. Estimated parameters are used throughout.
...
server <- function(input, output, session) {
  ...
  get_b <- function(K, spread) {
    seq(-(K - 2) / 2, (K - 2) / 2, length.out = K - 1) * spread
  }

  th_grid <- seq(-3, 3, length.out = 500)

  ## Analytical CRFs on theta grid for all three models
  all_probs <- reactive({
    K <- input$K; a <- input$a; b <- get_b(K, input$spread)
    list(
      grm = getprob("grm", th_grid, b, a),
  ...
  all_imp <- reactive({
    probs <- all_probs()
    list(
      grm = dichotomize_probs(probs$grm),
      pcm = dichotomize_probs(probs$pcm),
      srm = dichotomize_probs(probs$srm)
    )
  })

  dich_names <- reactive({ names(all_imp()[["grm"]]) })
  ...
  output$crf_plot <- renderPlot({
    ...
    mtext(paste0("CRFs — a = ", input$a, ", spread = ", input$spread),
          outer = TRUE, line = 0.3, cex = 1, family = "serif")
  }, res = 96)

  ## ── Tab 2: implied probability for selected dichotomization ──────────────
  output$imp_plot <- renderPlot({
    ...
    abline(h = 0.5, col = "gray80", lty = 2)
    ## draw thinnest first so thicker lines sit on top, thinner ones stay visible
    for (mod in c("srm", "pcm", "grm")) {
      ip <- imp[[mod]]
      if (nm %in% names(ip))
        lines(th_grid, ip[[nm]], col = cols[mod], lwd = lwds[mod], lty = ltys[mod])
    }
    legend("right", bty = "n",
           lwd = c(lwds["grm"], lwds["pcm"], lwds["srm"]),
           lty = c(ltys["grm"], ltys["pcm"], ltys["srm"]),
           col = c(cols["grm"], cols["pcm"], cols["srm"]),
           legend = c("GRM", "PCM", "SRM"), cex = 0.9)
  }, res = 96)

  ## ── Tab 3: IMV simulation ─────────────────────────────────────────────────

  ## Warning when DGM == DAM
  output$imv_warning <- renderUI({
    if (input$dgm == input$dam)
      div(style = "background:#fff3cd; border:1px solid #ffc107; border-radius:4px;
          ...
          exactly zero for every dichotomization — the DAM is estimated from responses
          generated by itself, so its predictions are identical to the DGM baseline.
          Select different models to see meaningful IMV values.")
  })

  imv_results <- eventReactive(input$run, {
    K <- input$K
    a <- input$a
    b <- get_b(K, input$spread)
    N <- 2000

    withProgress(message = "Running simulation...", value = 0, {

      ## Step 1: draw theta, simulate responses from DGM
      incProgress(0.15, detail = "Simulating responses from DGM")
      th   <- rnorm(N)
      resp <- simresp(input$dgm, th = th, b = b, a = a)

      ## Step 2: estimate both DGM and DAM parameters from those responses
      incProgress(0.25, detail = paste("Estimating", toupper(input$dgm), "parameters"))
      est_dgm <- tryCatch(
        estfun(input$dgm, resp, th),
        error = function(e) c(a, b)
      )

      incProgress(0.25, detail = paste("Estimating", toupper(input$dam), "parameters"))
      est_dam <- tryCatch(
        estfun(input$dam, resp, th),
        error = function(e) c(a, b)
      )

      ## Step 3: generate a fresh set of responses from DGM with estimated DGM parameters
      incProgress(0.15, detail = "Generating fresh responses from DGM")
      resp2 <- simresp(input$dgm, th = th, b = est_dgm[-1], a = est_dgm[1])

      ## Step 4: predictions — both models use their estimated parameters
      incProgress(0.15, detail = "Computing IMV")
      p_dgm <- getprob(input$dgm, th, b = est_dgm[-1], a = est_dgm[1])
      p_dam <- getprob(input$dam, th, b = est_dam[-1],  a = est_dam[1])

      ## IMV(DGM, DAM): p1 = DAM predictions (baseline), p2 = DGM predictions (enhanced)
      ## positive when DGM predicts better than DAM, as expected when DGM is correctly specified
  ...
  output$imv_summary <- renderPrint({
    ...
    cat("========================================\n\n")
    cat("Parameters: K =", input$K, "| a =", input$a, "| spread =", input$spread, "\n\n")
