...
get_coin_weight <- function(avg_ll, sigma = 1e-6) {
  ...
  # avg_ll is a geometric mean of likelihoods, so it's in (0,1)
  # w must be in (0.5, 1) — we always map to the "fair" side >= 0.5
...
imv_coins <- function(y, p_baseline, p_enhanced) {
  ...
  w0  <- get_coin_weight(ll0)
  w1  <- get_coin_weight(ll1)
  imv <- (w1 - w0) / w0
  list(w0 = w0, w1 = w1, imv = imv)
}
